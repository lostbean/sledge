...
genQuaternionGrid m =
    let
     ...
     in
        U.map (tesseractToQuaternion . Cell1) cell
            U.++ U.map (tesseractToQuaternion . Cell2) cell
            U.++ U.map (tesseractToQuaternion . Cell3) cell
            U.++ U.map (tesseractToQuaternion . Cell4) cell
...
accuTesseract qs vs tess@TesseractGrid{..} =
    let
...
updateTesseract qs vs tess@TesseractGrid{..} =
    let
        us = V.map (getTesseractPos gridSize . quaternionToTesseract) $ V.take (V.length vs) qs
        vp = V.map (tessPosToIx gridSize . snd) us
        func cell i =
...
tesseractMap func ta =
    ta
        { cell1 = V.map func (cell1 ta)
...
maxTesseractPoint TesseractGrid{..} =
    let
        m1 = V.maxIndex cell1
        m2 = V.maxIndex cell2
        m3 = V.maxIndex cell3
     ...
     in
        (getTesseractPoint gridSize (c + 1, ixToTessPos gridSize $ ms V.! c))
...
plotTesseract t =
    let
     ...
     in
        mkSPVTK "Tesseract" (m + 1, m + 1, m + 1) (orig, orig, orig) (step, step, step) attrs

plotTesseractPoints :: U.Vector Quaternion -> VTK Vec3D
plotTesseractPoints qs =
    let
        ...
        foo x = case x of
            ...
            Cell4 v -> (4, v)
     in
        mkUGVTK "Tesseract" ps (V.generate (U.length ps) id) [attr] []
...
printTesseract t fname =
    let
        vtk = plotTesseract t
     in
        writeUniVTKfile ("/home/edgar/Desktop/" ++ fname ++ ".vti") True vtk
