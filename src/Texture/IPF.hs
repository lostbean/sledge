...
getRGBColor (RGBDoubleColor (r, g, b)) =
    let
        ru = round $ r * 255
        gu = round $ g * 255
        bu = round $ b * 255
    ...
    let
        func plane x
          ...
          where
            alpha = angle' plane x
     in
        AngularDist (func planeRG v, func planeGB v, func planeBR v)
...
findMinAngleBase symOps base x =
    let
