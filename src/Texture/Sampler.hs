...
data HRSCfg
    = HRSCfg
    { initShotDist :: Double
    -- ^ Initial absolute distance
    , mixingFraction :: Double
    {- ^ Mixing factor. 0 means always use 'initShotDist' and
    1 means always use last range
    -}
    , longShotFraction :: Double
    -- ^ Fraction of frequency of long shot ('maxShotDist') tries.
    , maxShotDist :: Double
...
rangeMixer HRSCfg{..} km (rl, ru) =
    let
        m = min 1 (abs mixingFraction)
        avgk = 0.5 * (abs rl + abs ru)
     in
        modifyIORef km $ \k -> (1 - m) * k + m * avgk
