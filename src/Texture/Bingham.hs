...
data Bingham
    = Bingham
    { concentrations :: Vec4D
    {- ^ concentration values for each directions where
    z1 <= z2 <= z3 <= (z4 = 0)
    -}
    , directions :: DistOri
    -- ^ unitary vectors Vx in rows
    , bingMatrix :: Mat4D
    ...
    , partials :: DFDzx
    -- ^ partials derivates of F in each direction
    , scatter :: Mat4D
    -- ^ scatter matrix
    }
    deriving (Show)
    ...
    func acc t0 p0 q0 count accepted = do
        ...
        let
            ...
            out rnd
                ...
                | count < 0
                    && a > rnd =
                    func acc t p q (count + 1) accepted
...
quaternionNormalRand z cv q =
    let
        foo = mkQuaternion . (&+ quaterVec q)
     in
        foo <$> multiNormalRand z cv
...
evaldFF :: Vec3D -> DFF
evaldFF (Vec3 z1 z2 z3) =
    let
        (f, (dFdz1, dFdz2, dFdz3, dFdz4)) = computeAllF z1 z2 z3 0
     in
        DFF $ Vec4 (dFdz1 / f) (dFdz2 / f) (dFdz3 / f) (dFdz4 / f)
...
renderBinghamToEuler step (np1, np, np2) dist =
    let
        start = toAngle 0
        angs = V.enumFromStepN start step (max np1 (max np np2))
        eu =
            [ mkEuler (angs V.! i) (angs V.! j) (angs V.! k)
            | i <- [0 .. np1 - 1]
            , j <- [0 .. np - 1]
            , k <- [0 .. np2 - 1]
            ]
