...
instance Random Quaternion where
    ...
    randomR (_, _) gen =
        let
         ...
         in
            (mkQuaternion $ Vec4 q0 q1 q2 q3, gen1)
...
instance Rot Quaternion where
    p #<= q =
        let
         ...
         in
            unsafeMergeQuaternion (pq0, pq)
    invert q =
        let
            (q0, qv) = splitQuaternion q
         in
            unsafeMergeQuaternion (q0, neg qv)
    toQuaternion = id
    fromQuaternion = id
...
instance Rot AxisPair where
    ...
    toQuaternion (AxisPair (r, omega)) =
        let
            q0 = cos (0.5 * omega)
            q = r &* sin (0.5 * omega)
         in
            unsafeMergeQuaternion (q0, q)
    fromQuaternion q
...
aproxToIdealAxis (Vec3 x y z) err =
    let
        ls = [x, y, z]
...
instance Show AxisPair where
    show (AxisPair (vec, theta)) =
        let
         in -- (x, y, z) = aproxToIdealAxis vec 0.05
            showPretty vec ++ " @ " ++ show (toAngle theta :: Deg)
...
instance Show Euler where
    show Euler{..} =
        let
         ...
         in
            "Euler(" ++ foo phi1 ++ ", " ++ foo phi ++ ", " ++ foo phi2 ++ ")"
