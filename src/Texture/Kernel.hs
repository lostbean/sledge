...
addKernelWithConstM :: (Metric p, U.Unbox p, Angle a, GM.MVector v Double)
                    => a -> Double -> VPtree p -> p -> v s Double -> ST s ()
...
addOneKernelWithConst :: (Metric p, U.Unbox p, Angle a, G.Vector v Double)
                      => a -> Double -> VPtree p -> p -> v Double -> v Double
addOneKernelWithConst  a k tree p = G.modify (addKernelWithConstM a k tree p)

addManyKernelsWithConst :: (Metric p, U.Unbox p, Angle a, G.Vector v p, G.Vector v Double)
