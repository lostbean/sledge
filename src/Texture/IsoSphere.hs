...
addAddGetNewPos ::
    (UM.Unbox a, Num a) =>
    (STRef s a, VM.MVector s (U.Vector (Int, a))) ->
    (Int, Int) ->
    ST s a
...
nearestPoint IsoSphere{..} q = getClosest 0 fid0
  where
    ...
    getClosestOnFace (ia, ib, ic) =
        let
            xs = map (\i -> let v = vertices U.! i in (i, v, getD v)) [ia, ib, ic]
         in
            L.minimumBy (compare `on` (\(_, _, d) -> d)) xs
