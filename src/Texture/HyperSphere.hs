...
cartToSO2 n =
    let
        s = vlen n
     in
        SO2{so2Theta = acos (_3 n / s), so2Phi = atan2 (_2 n) (_1 n)}
    ...
    let
        ...
        mkStrip l1 l2 =
            let
             ...
             in
                merge `U.cons` U.izipWith func (U.init l1) (U.init l2)
        foo acc i l = acc U.++ mkStrip l (ll V.! (i + 1))
     in
        V.ifoldl' foo U.empty (V.init ll)
...
getSO3IsoGrid nOmega nTheta nPhi =
    let
...
getSO3Cells nOmega nTheta nPhi =
    let
        ...
        func i (SO2Cell (a, b, c, d)) =
            let
                k1 = psSize * i
                k2 = psSize * (i + 1)
             in
                SO3Cell ((a + k1, b + k1, c + k1, d + k1), (a + k2, b + k2, c + k2, d + k2))
     in
        U.concatMap (\i -> U.map (func i) so2cells) layers
...
renderSO2VTK feval =
    let
     ...
     in
        addPointValueAttr vtk attr

renderSO3SolidVTK :: (SO3 -> Double) -> VTK Vec3D
renderSO3SolidVTK feval =
    let
