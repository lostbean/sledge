        ...
        let
            ...
            mkOne i =
                let
                 ...
                 in
                    SymmOp $ toQuaternion $ mkRodrigues v (Rad omega)
         in
            U.generate (nfold - 1) mkOne
...
isInFZ symm = \q ->
    let
     ...
     in
        ...
        isNothing $ U.find ((> w) . abs . composeQ0 q . symmOp) os
  where
    os = getSymmOps symm
...
isInRodriFZ symm = \q ->
    let
     ...
     in
        ...
        isNothing $ U.find (not . isInsideRFplane rq) planes
  where
