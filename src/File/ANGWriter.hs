...
renderANGFile fileName ang =
    let
        txt = toLazyByteString $ renderANGdata ang
     in
        BSL.writeFile fileName txt
...
headRenderA ANGgrid{..} ANGinfo{..} =
    let
        (xstart, ystart, zstart) = origin
     in
        (render ("# TEM_PIXperUM" :: Text) <> tab <> render pixperum)
    ...
    let
        txt =
            renderF 3 a
                ...
                <-> renderF 3 w3
     in
        setInfo "# LatticeConstants" txt

elasticRender :: (Double, Double, Double, Double, Double, Double) -> Builder
elasticRender (i1, i2, i3, i4, i5, i6) =
    let
        txt = i1 <-> i2 <-> i3 <-> i4 <-> i5 <-> i6
     in
        setInfo "# ElasticConstants" txt
    ...
    let
        foo (h, k, l, a, b, c) =
            let
                txt = h <-> k <-> l <-> a <-> b <-> c
             in
                setInfo "# hklFamilies" txt
     in
        mconcat $ map foo hkl

categoryRender :: (Int, Int, Int, Int, Int) -> Builder
categoryRender (a, b, c, d, f) =
    let
        txt = a <-> b <-> c <-> d <-> f
     in
        setInfo "# Categories" txt
...
getGridPoint g (xi, yi) =
    let
        (xstep, ystep) = xystep g
     in
        (fromIntegral xi * xstep, fromIntegral yi * ystep)
