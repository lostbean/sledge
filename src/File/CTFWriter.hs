...
renderCTFdata CTFdata{..} =
    let
        CTFinfo{..} = ebsdInfo
        ps = map phaseRender phases
     in
        render ("Channel Text File" :: Text)
            <> setInfo "Prj" project
...
phaseRender CTFphase{..} =
    let
        is = map ((tab <>) . render) phaseInfo
     in
        setInfo "Phases" phaseID
            <> eol
...
latticeRender (a, b, c, alpha1, alpha2, alpha3) =
    let
        sep = fromChar ';'
