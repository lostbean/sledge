...
import Texture.Orientation (Deg (..), Quaternion, mkEuler, toQuaternion)
...
data CTFpoint
    ...
    , errorID :: {-# UNPACK #-} !Int
    -- ^ error
    , angulardev :: {-# UNPACK #-} !Double
    -- ^ MAD (mean angular deviation)
    , bandcontrast :: {-# UNPACK #-} !Int
    -- ^ BC  (band contrast)
    , bandslope :: {-# UNPACK #-} !Int
    -- ^ BS  (band slope)
    }
    deriving (Show)
...
checkDataSize CTFdata{..}
  ...
  where
    ...
    msg =
        error $
            "[CTF] Invalid numbers of points. Expected "
                ++ show n
                ++ " but found "
                ++ show (V.length nodes)
                ++ ". The grid shape is given by (row, cEven, cOdd) = "
                ++ show rowCols
